...
class OVSDB():
    ...
    @decorator.check_arg
    def find(self, obj_type, condition):
        cmd = 'ovs-vsctl find {0} {1}'.format(obj_type, condition)
        result, error = Popen(cmd, stdout=PIPE, stderr=PIPE, shell=True).communicate()
    ...
    @decorator.check_arg
    def add(self, obj_type, obj_name, obj_data):
        cmd = 'ovs-vsctl add {0} {1}'.format(obj_type, obj_name)
        for key in obj_data:
            cmd += ' {0}={1}'.format(key, obj_data.get(key))
        _, error = Popen(cmd, stdout=PIPE, stderr=PIPE, shell=True).communicate()
        return False if error else True
    ...
    @decorator.check_arg
    def remove(self, obj_type, obj_name, obj_key):
        cmd = 'ovs-vsctl remove {0} {1} {2}'.format(obj_type, obj_name, obj_key)
        result, error = Popen(cmd, stdout=PIPE, stderr=PIPE, shell=True).communicate()
        return result.strip() if not error else None
